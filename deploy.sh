...
echo "==> Setting Infra queue's status list (Open, In Progress, Resolved)..."
...
# status list (applies to all roles, per queue status scoping in the app) and is safe to
# re-run on every deploy — it only touches the "Infra" queue entry's queueStatuses field.
docker exec -i jira_postgres psql -U jirauser -d jiradb <<'SQL' 2>&1 | grep -v NOTICE || true
UPDATE custom_queues
SET queues = (
  SELECT jsonb_agg(
    CASE WHEN lower(elem->>'name') = 'infra'
      THEN elem || jsonb_build_object('queueStatuses', '[
        ...
        {"id":"qst_infra_resolved","name":"Resolved","color":"#10B981","category":"done","order":2}
      ]'::jsonb)
      ELSE elem
    END
  )
...
WHERE EXISTS (
  SELECT 1 FROM jsonb_array_elements(queues) e WHERE lower(e->>'name') = 'infra'
);
SQL
