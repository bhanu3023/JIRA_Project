...
import { AlertTriangle, RefreshCw, Clock, ChevronDown, X, Check } from 'lucide-react';
...
function MultiSelect({
  label, options, selected, onChange, colorClass = 'bg-blue-50 text-blue-700 border-blue-200',
}: {
  label: string;
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className={cn(
          'flex items-center gap-2 text-xs font-medium border rounded-lg px-3 py-2 transition-colors min-w-[130px] bg-white',
      ...
      {open && (
        <div className="absolute top-full left-0 z-50 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg py-1 min-w-[170px]">
          {options.map(o => (
            <button
              key={o.value}
...
export default function BreachedPage() {
          ...
          for (const issue of openIssues) {
            ...
            for (const policy of slaPolicies) {
              ...
              if (dueTime < now) {
                ...
                // Extract product type from custom field values or issue field
  ...
  const filtered = React.useMemo(() => {
    return allIssues.filter(i => {
      if (selectedQueues.length > 0 && !selectedQueues.includes(i.spaceKey)) return false;
  ...
  }, [allIssues, selectedQueues, selectedProducts]);
  ...
  const queueOptions = spaces.map(s => ({ value: s.key, label: s.name }));
  const productOptions = PRODUCT_TYPES.map(p => ({ value: p, label: p }));
  const hasFilters = selectedQueues.length > 0 || selectedProducts.length > 0;
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50">
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          ...
          {hasFilters && (
            <button
              onClick={() => { setSelectedQueues([]); setSelectedProducts([]); }}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">
              {hasFilters ? 'Try clearing the queue or product type filter' : 'All queue tickets are within their SLA targets'}
            </p>
          </div>
        ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      ...
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-2 py-2 w-28">Product Type</th>
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-2 py-2 w-28">Assignee</th>
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-2 py-2 w-32">Overdue by</th>
                    </tr>
                  </thead>
                  <tbody>
                    {spaceIssues.map(issue => (
                      <tr key={issue.key} className="border-b border-gray-50 hover:bg-red-50/30 transition-colors">
                        ...
                        <td className="px-2 py-2.5">
                          {issue.assignee ? (
                            <span className="text-xs text-gray-600 truncate block max-w-[110px]">{issue.assignee.displayName}</span>
                          ) : (
