          ...
          <div className={`px-4 py-3 space-y-0 ${!canEdit ? 'pointer-events-none opacity-70' : ''}`}>
            ...
            {customFields.filter(cf => pinnedFields.includes(`cf_${cf.id}`) && cf.fieldType !== 'department-routing' && cf.type !== 'department-routing').map(cf => {
              const KNOWN_CF_OPTIONS: Record<string, string[]> = {
                ...
                'Project Manager': ['Abhishek','Abhishikth','Ajay Singh','Chandra Mouli','Harika','Lakshmi Prasanna','Meghana','Neelima','Raghu','Sri Ram','Sravan','Pranavi'],
            ...
            {issue.spaceKey === 'L2BOARD' && (() => {
              ...
              const l2bFields: { key: string; label: string; type: 'select' | 'multiselect' | 'textarea'; options?: string[] }[] = [
                ...
                { key: 'projectManager', label: 'Project Manager',  type: 'multiselect', options: ['Harika','Abhishek','Ajay Singh','Abhishikth','Raghu','Lakshmi Prasanna','Sri Ram','Chandra Mouli','Sravan','Pranavi','Meghana','Neelima','Others'] },
                  ...
                  {startedAt && (
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      <div className="bg-gray-50 rounded-lg px-2.5 py-1.5">
                        ...
                        {/* Reverted per explicit request: each department's own
                            SLA should show when THAT department actually
                            received the ticket, not the ticket's original
                            creation date -- e.g. Migration created at 10:07 PM,
                            routed to Dev at 11:00 PM, Dev's own SLA Start should
                            read 11:00 PM, not 10:07 PM. `startedAt` already holds
                            exactly that (dept_sla_started_at), matching the
                            Due-time math right next to it. */}
