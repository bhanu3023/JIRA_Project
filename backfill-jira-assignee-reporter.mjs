/**
 * backfill-jira-assignee-reporter.mjs
 *
 * Fills in missing assignee/reporter for L2B-* and L3B-* tickets from live
 * Jira, for whichever field is currently NULL. Deliberately additive only:
 * it never overwrites a ticket that already has an assignee or reporter set
 * (whether that came from the original migration or a real reassignment
 * done inside the app since then) -- only a currently-empty field gets
 * filled in.
 *
 ...
 *
 * Run: JIRA_EMAIL=... JIRA_TOKEN=... node backfill-jira-assignee-reporter.mjs
 */
import pg from 'pg';
...
async function resolveOrCreateUser(jiraUser) {
  ...
  // in the API response at all, which is exactly why these tickets' reporter
  // ended up NULL in the first place -- the original migration had nowhere
  // to match or create a user without one. Synthesize a stable placeholder
  // email from the account id instead of skipping, matching the same
  // fallback sync-all-assignees.mjs already uses for this exact situation.
  ...
  if (!email) return null; // no email and no account id to key a placeholder off of -- nothing safe to do
  ...
  const newId = rid();
  // No unique constraint on users.email in this schema, so ON CONFLICT isn't
  // available here -- two concurrent workers resolving the same brand-new
  // email at once is possible but rare at this scale; re-check after a
  // failed insert instead of adding a constraint as a side effect of this
  // script.
  try {
    await pool.query(
...
const stats = { processed: 0, assigneeFilled: 0, reporterFilled: 0, errors: 0 };

async function backfillIssue(id, key, needsAssignee, needsReporter) {
  const wanted = needsAssignee && needsReporter ? 'assignee,reporter' : needsAssignee ? 'assignee' : 'reporter';
  const data = await jiraGetWithRetry(`/rest/api/3/issue/${key}?fields=${wanted}`);
...
async function main() {
  const res = await pool.query(
    `SELECT id, key, "assigneeId", "reporterId" FROM issues
     WHERE (key LIKE 'L2B-%' OR key LIKE 'L3B-%') AND ("assigneeId" IS NULL OR "reporterId" IS NULL)
     ORDER BY key`
  );
  const targets = res.rows.slice(0, LIMIT === Infinity ? res.rows.length : LIMIT);
  console.log(`Found ${res.rows.length} L2B/L3B tickets missing assignee and/or reporter; processing ${targets.length} this run with concurrency=${CONCURRENCY}.`);

  await runPool(targets, async (row) => {
    try {
      await backfillIssue(row.id, row.key, row.assigneeId === null, row.reporterId === null);
      stats.processed++;
    } catch (e) {
      stats.errors++;
    ...
    if (stats.processed % 25 === 0) {
      console.log(`progress: ${stats.processed}/${targets.length} | +assignee=${stats.assigneeFilled} +reporter=${stats.reporterFilled} errors=${stats.errors}`);
    }
  }, CONCURRENCY);
